...
<img src="docs/images/menubar.png" alt="菜单栏显示当前价、涨跌幅与持仓收益" width="180">

就是菜单栏上这一小条。显示哪些数据、什么格式，都可以自己定。
...
<img src="docs/images/dropdown.png" alt="下拉列表展示全部股票与多币种合计" width="420">

最多 8 只股票，一次请求批量刷新（固定 3 秒），点任意行即可切换菜单栏显示哪只。

**多币种按币种分开合计，绝不相加**——没有汇率数据源，港币与人民币混加只会得出看似精确的错误数字。

### 到价提醒

<img src="docs/images/notification.png" alt="提醒触发时的 macOS 通知横幅" width="520">

价格、涨跌幅、持仓收益到达阈值时发系统通知。采用**穿越触发**——回落后再次越过才响，不会在阈值附近反复轰炸。

通知文案可完全自定义（比如换成「今天吃了三斤肉」），也可以设为静默。休市期间用「试发」就能验证通知链路，不必等开盘。

<img src="docs/images/settings-alerts.png" alt="提醒规则设置页" width="760">

### 菜单栏显示什么，自己定

<img src="docs/images/settings-display.png" alt="菜单栏显示项配置页，含实时预览" width="760">

16 种数据项任意勾选与排序，精度、正负号、万/亿缩写逐项可调，改动即时反映在顶部预览里。

按交易时段与行情时间戳区分 交易中 / 休市 / 延迟（A 股与港股分别判定）。

### 股票与持仓

<img src="docs/images/settings-stock.png" alt="股票列表管理页" width="760">

搜索添加，拖动排序，列表顺序即菜单栏下拉的顺序。每只股票可单独填写持仓数量与成本，收益全部在本地算。
